...
/* ── GOOGLE SHEETS LOGGER ──
   Logs every signup and login to your Google Sheet.
   Replace the URL below with your own Apps Script Web App URL.
   How to get it: Extensions → Apps Script → Deploy → New Deployment
   → Web App → Execute as Me → Anyone → Deploy → copy URL
──────────────────────────────────────────────────────────── */
...
function logToSheet(username, action){
  if(!SHEET_URL || SHEET_URL.includes('PASTE_YOUR')) return; /* skip if not set */
  ...
  fetch(SHEET_URL,{
    method:'POST',
    mode:'no-cors', /* required for Apps Script — response won't be readable but data IS saved */
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({username, action, device, browser})
  }).catch(()=>{}); /* silent fail — never breaks the app */
}

/* ───────────────────────────────────────────────────────
   FIREBASE HELPERS
   ─ Signup         → instant, zero cloud wait
   ─ Same device    → instant local data, cloud syncs quietly
   ─ New device     → waits for cloud (shows spinner — expected)
──────────────────────────────────────────────────────── */
...
async function fbGet(collection, docId){
  try{
    const snap=await withTimeout(
      window._fsGetDoc(window._fsDoc(window._db, collection, docId)), 10000
    );
...
function fbSetBg(collection, docId, data){
  try{
    window._fsSetDoc(window._fsDoc(window._db,collection,docId),data,{merge:true})
      .catch(()=>{});
  }catch(e){}
}
...
async function syncUsersFromCloud(){
  ...
  if(cloud&&cloud.users){
    ...
    const merged=Object.assign({},cloud.users,local);
    localStorage.setItem('ev_users',JSON.stringify(merged));
    return merged;
  }
...
function saveUsers(u){
  saveUsersLocal(u);
  fbSetBg('ev_users','__accounts__',{users:u});
}

/* ── MEMORY HELPERS ── */
function getMemLocal(uid){
  return JSON.parse(localStorage.getItem('ev_mem_'+uid)||'[]');
}

/*
  Smart getMem:
  - New signup      → return [] instantly, skip cloud
  - Same device     → return local instantly, silently pull cloud in background
  - New device      → no local data, MUST await cloud (shows spinner)
*/
async function getMem(uid){
  /* 1. Brand new signup — skip cloud entirely */
  ...
  /* 2. Same device — has local data: show instantly, sync in background */
  if(localMem.length>0){
    bgSyncMemories(uid);   /* silent background refresh */
    return localMem;
  }

  /* 3. New device — no local data: must fetch from cloud */
  const cloud=await fbGet('ev_memories',uid);
  if(cloud&&cloud.memories&&cloud.memories.length>0){
...
/*
  Background memory sync — runs after vault opens on same device.
  Quietly pulls any new memories saved from other devices.
*/
function bgSyncMemories(uid){
  /* Use raw promise with no timeout so it doesn't race with UI */
  try{
    window._fsGetDoc(window._fsDoc(window._db,'ev_memories',uid))
      .then(snap=>{
        if(!snap.exists())return;
        const cloud=snap.data();
        if(!cloud||!cloud.memories)return;
        const local=getMemLocal(uid);
        /* Only update if cloud has more memories than local */
        if(cloud.memories.length>local.length){
          memories=cloud.memories;
          localStorage.setItem('ev_mem_'+uid,JSON.stringify(cloud.memories));
          refresh();
          toast('Vault synced — new memories loaded.');
        }
      })
      .catch(()=>{});
  }catch(e){}
...
/* Save locally now, push to cloud in background */
function saveMem(uid,m){
  localStorage.setItem('ev_mem_'+uid,JSON.stringify(m));
  fbSetBg('ev_memories',uid,{memories:m});
}
...
function showLoading(msg='Syncing…'){
  ...
  if(!el){
    ...
    el.innerHTML=`
      <div style="width:56px;height:56px;border:4px solid rgba(255,255,255,.15);border-top-color:var(--accent);border-radius:50%;animation:spin .8s linear infinite"></div>
      <span id="fbLoaderMsg" style="opacity:.85">${msg}</span>
      <span style="font-size:.72rem;opacity:.45;font-family:var(--fm)">Connecting to cloud…</span>`;
    if(!document.querySelector('#fbSpinKf')){
      const s=document.createElement('style');s.id='fbSpinKf';
...
/* ── LOGIN ──
   Same device  → instant local check, no cloud wait
   New device   → fetch from cloud (shows brief spinner)
*/
async function doLogin(){
  ...
  /* Same device: check locally first — instant */
  let users=getUsersLocal();
  if(users[u]){
    if(users[u].hash!==simpleHash(p)){authErr('Wrong password. Try again!');return}
    logToSheet(u,'login'); /* ← log to sheet */
    await loginUser(u);
    return;
  }

  /* New device: account not cached locally — fetch from cloud */
  showLoading('Fetching your account…');
...
/* ── SIGNUP ──
   Saves locally and enters app instantly.
...
async function doSignup(){
  ...
  /* Check locally first (instant) */
  const users=getUsersLocal();
  if(users[u]){authErr('Username taken! Try another one.');return}

  /* Save locally + background cloud sync */
  users[u]={hash:simpleHash(p),created:Date.now()};
  saveUsers(users);

  logToSheet(u,'signup'); /* ← log to sheet */

  /* Tell getMem to skip cloud fetch — brand new user has no memories */
  sessionStorage.setItem('ev_newuser',u);
...
/*
  loginUser:
  - Same device with data  → instant (getMem returns local)
  - New device             → brief spinner while cloud loads
*/
async function loginUser(u){
  const hasLocalMem=getMemLocal(u).length>0;
  const isNew=sessionStorage.getItem('ev_newuser')===u;

  /* Only show spinner if this device has no cached memories
     AND it's not a brand new signup */
...
async function tryAutoLogin(){
  ...
  if(localUsers[s]){
    /* Same device — go instantly */
    await loginUser(s);
    return;
  }
  /* New device with saved session — verify against cloud */
  showLoading('Checking your account…');
  const cloudUsers=await syncUsersFromCloud();
  hideLoading();
  if(!cloudUsers[s]){
...
/* Wait for Firebase module, then try auto-login */
